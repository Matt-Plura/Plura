...
<Steps>
  <Step title="Request Method & URL ">
    Selects the HTTP method (GET, POST, etc.) and specifies the API endpoint.
  </Step>
  <Step title="Authentication">
    Defines the authentication method required for the API (None, Basic Auth, API Key, etc.).
  </Step>
  <Step title="Content Type">
    Specifies the format of the request payload (e.g., JSON, Form Data, XML).
  </Step>
  <Step title="Response Type">
    Determines the expected format of the API response (e.g., JSON, XML, plain text).
  </Step>
  <Step title="Headers ">
    Allows adding custom headers required for authentication or API-specific configurations.
  </Step>
  <Step title="Request Body">
    Defines the data sent in the API request, applicable for POST, PUT, and PATCH requests.
  </Step>
  <Step title="Test Request">
    Provides a way to test the API call setup before integrating it into the workflow
  </Step>
</Steps>
